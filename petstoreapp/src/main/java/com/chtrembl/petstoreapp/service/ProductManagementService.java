    ...
    public Collection<Product> getProductsByCategory(String category, List<Tag> tags) throws Exception {
       ...
       /* List<Product> products;
        ...
        try {
            ...
            // Custom event with username and session info
            Map<String, String> eventProps = new HashMap<>();
            eventProps.put("username", this.sessionUser.getName());
            eventProps.put("sessionId", this.sessionUser.getSessionId());

            this.sessionUser.getTelemetryClient().trackEvent(
                    "Product retrieval requested",
                    eventProps,
                    null
            );
            ...
            // Log and custom metric for number of items returned
            int productCount = products.size();
            log.info("Returned {} products to user {} [RequestID: {}, TraceID: {}]", productCount, this.sessionUser.getName(), requestId, traceId);
